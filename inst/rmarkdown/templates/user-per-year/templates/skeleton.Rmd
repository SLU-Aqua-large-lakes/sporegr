...
cpue_per_date_plot <- cpuedata %>% 
  ...
  #geom_bar(stat="identity", aes(col=MALART)) +
    geom_line(aes(col=FANGOMR)) + geom_point(size=2, aes(col=FANGOMR)) + 
  facet_grid(scales = "free_y", rows = vars(MALART)) +
  scale_x_date(date_breaks = "day" , date_labels = "%b-%d", guide = guide_axis(angle = 60)) +
  #scale_y_continuous(breaks = integer_breaks()) +
  theme(legend.position='bottom') + ylab("Fångstindex (antal per spötimme)") + xlab("")
...
resa_summary <- t1 %>%
  ...
  select(Resa = NAMN, Datum = RESEDATUM, Målart = MALART, Område = FANGOMR, Personer = ANTALPERSONER,
    ...
    # Lägger till ett fångstindex:                                                        ##### MEN TABELLEN BLIR BRED
    mutate(`CPUE (antal per spötimme)` = round(`Fångst (målart)` / (Minuter/60*Personer),2))
...
cap <- "Fångster under perioden. Antal fiskar samt största exemplar (mm)"
knitr::kable(fangst_per_art, caption = cap)
```
## Fångster per dag

Antal fångade fiskar per resa.
...
Fångstindex beräknat som fångst per ansträngning. Fångst är beräknat som antal fångade fiskar av målarten och ansträngning som spötimmar (antal personer * antal timmar). 


```{r cpue_per_date_plot, echo=FALSE, message=F}
if (have_fangst) {
