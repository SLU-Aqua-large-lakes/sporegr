...
### Set up control, only used locally! ##
# Detta laddar alla funktioner från R/-mappen in i "Knit"-sessionen
library(devtools)
devtools::load_all() 
### End local set up
...
per_date_plot <- fangst_resa %>%
  ...
  ggplot(., aes(x = Date, y = Antal, fill=ARTBEST)) +
  geom_col(col="black") +
...
if (nrow(cpuedata) > 0) {
  cpue_per_date_plot <- cpuedata %>%
    ggplot(., aes(x = RESEDATUM, y = cpue, col = FANGOMR)) +
...
if (nrow(length_dist) > 0) {
  length_plot <- ggplot(data = length_dist, mapping = aes(x = LANGD, fill=ARTBEST)) +
    geom_histogram( binwidth = 2.5, boundary=0, closed="left",color="black") +
    scale_fill_sporeg_art() + 
