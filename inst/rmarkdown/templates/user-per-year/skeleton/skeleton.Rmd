  ...
  left_join(t2, by = c("UUID" = "UUID", "MALART" = "ARTBEST")) %>%
   mutate(Resa =  substr(NAMN, 1, 15)) %>%
     select(Resa, Datum = RESEDATUM,
            ...
            Minuter = FISKEMINUTER, `Fångst (målart)` = Antal,  ## Kan vi räkna om minuter till HH:MM? Klurigt.
...
t3 <- fangst %>% rename(Art = ARTBEST) %>% dplyr::group_by(Art, ATERUTSATT) %>% dplyr::summarize(N=n()) %>% 
