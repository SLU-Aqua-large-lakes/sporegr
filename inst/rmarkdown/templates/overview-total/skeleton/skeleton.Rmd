...
library(ggthemes)
library(janitor)
source("C:/R/sporegr/sporegr/R/gg_color_sporeg.R", encoding="UTF-8") # Ensure color by FANGOMR
...
cpue_tab <- cpuedata %>%
  rename(Art = MALART,
         Område = FANGOMR) %>%
  dplyr::group_by(Art, Område, EFTERREGISTRERING) %>%
  dplyr::summarize(N=n(), 
...
cap <- "CPUE per art och område. N anger antal fisketurer som ligger till grund för beräkningen (färre än 10 fisketurer är exkluderade)"
...
#### Test plot: Skillnad i cpue med/utan efterreg?
### Ser ut att kunna finnas något där faktiskt...? 
#cpue_tab %>% 
#  ggplot(aes(x=EFTERREGISTRERING, y=Median, col=Område)) + 
#  geom_point(size=3) + 
#  facet_wrap(~Art, scales="free") +
#  scale_color_sporeg() + 
#  geom_errorbar(aes(ymin=Median-SD, ymax=Median+SD))
### Hur ser andel turer utan fångst ut:
#cpuedata %>% mutate(bomtur = ifelse(cpue>0, 0, 1)) %>% count(MALART, EFTERREGISTRERING, bomtur) %>% pivot_wider(names_from=bomtur, #values_from=n) %>% mutate(andel.bom = `1`/(`1`+`0`))
## för det mesta ganska lika. Låter därför detta vara avmarkerat så länge, men kvar i dokumentet inför framtiden.

# Titta på fördelningen?
fig.cpue.freq <- 
  cpuedata %>% filter(MALART %in% unique(cpue_tab$Art)) %>%
  ggplot(aes(cpue)) + 
  geom_histogram(closed="left", color="black", boundary=0) + 
  geom_freqpoly() + 
