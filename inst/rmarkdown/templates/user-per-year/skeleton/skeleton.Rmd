...
map <-  raster::raster(map_os)
bgmap <- ggplot2::ggplot() +
...
## Peterguide kommenterar att medellängd och längsta kan vara bra i tabellen. Testar ersätta cpue med längsta per tur
 resa_summary <- t1 %>%
  left_join(t2, by = c("UUID" = "UUID", "MALART" = "ARTBEST")) %>%
     select(Resa = NAMN, Datum = RESEDATUM, Målart = MALART,
...
per_date_plot <- fangst_resa %>%
  ...
  ggplot(., mapping = aes(x = RESEDATUM, y = Antal,
                          fill = ARTBEST)) +
  geom_col() +
...
cpue_per_date_plot <- cpuedata %>%
  ...
  geom_line(aes(col=FANGOMR)) +
  geom_point(size=2, aes(col=FANGOMR)) + 
